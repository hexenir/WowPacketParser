...
// ReSharper restore InconsistentNaming
        ...
        public PacketReader(SniffType type, BinaryReader reader)
        {
            _sniffType = type;
            _reader = reader;
            ReadHeader();
        ...
        public Packet Read(int number, string fileName)
        {
                    ...
                    case PktVersion.V3_1:
                    {
                        ...
                        if (additionalSize >= 20 && _snifferId == 'T' && _pktVersion == PktVersion.V3_1)
                        {
                            ...
                            endPoint = new IPEndPoint(
                                new IPAddress(Array.TrueForAll(secondPartIpBytes, b => b == 0)
                                    ? firstPartIpBytes
                                    : realIpBytes), port);
                            ...
                            if (_snifferVersion >= 0x101)
                            {
                                ...
                                if (commentLength > 0)
                                {
                                    if (Settings.DumpFormatWithText())
                                    {
                                        ...
                                        writer.AppendLine("# " +
                                                          Encoding.UTF8.GetString(_reader.ReadBytes(commentLength)));
                                        writer.AppendLine();
                                    }
                                }
                            }
            ...
            if (opcode >= 1312 && (ClientVersion.Build <= ClientVersionBuild.V3_3_5a_12340 &&
                                   ClientVersion.Build != ClientVersionBuild.Zero))
                return null;
